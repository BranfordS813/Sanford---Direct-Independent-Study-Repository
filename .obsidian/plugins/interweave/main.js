...
var Relation = class {
  constructor(a, b, shouldIntersect) {
    this.a = a;
    this.b = b;
    this.shouldIntersect = shouldIntersect;
  }
};
var SetOperationNode = class {
...
var SetOperation = class {
  constructor(a, b, operation, color, opacity) {
    this.a = a;
    this.b = b;
...
var Renderer = class {
  static drawDifference(ctx, radius2, operation) {
    ...
    bctx.beginPath();
    bctx.arc(operation.a.x, operation.a.y, radius2, 0, Math.PI * 2);
    bctx.fill();
    bctx.globalCompositeOperation = "destination-out";
    for (let i = 0; i < operation.b.length; i++) {
      const item = operation.b[i];
      bctx.beginPath();
      bctx.arc(item.x, item.y, radius2, 0, Math.PI * 2);
      bctx.fill();
    }
  ...
  static drawFullCircle(ctx, pos, radius2, color, alpha) {
    ctx.beginPath();
  ...
  static drawText(ctx, pos, text) {
    ctx.font = "20px Arial";
    const metrics = ctx.measureText(text);
    ctx.fillStyle = "white";
    ctx.fillText(text, pos.x - metrics.width / 2, pos.y);
  }
  static drawClip(ctx, pos, radius2) {
    ctx.beginPath();
    ctx.arc(pos.x, pos.y, radius2, 0, Math.PI * 2);
    ctx.clip();
  }
  static drawSegment(ctx, renders, radius2, color, alpha) {
    ctx.save();
    for (let i = 0; i < renders.length - 1; i++) {
      Renderer.drawClip(ctx, renders[i].pos, radius2);
    }
  ...
  static render(data, canvas) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (item.sets.length == 1) {
        ...
        let operationType = 0 /* None */;
        let operationSets = [];
        let operationColor = "";
        let operationOpacity = 1;
        if (item.subtract != void 0) {
          operationType = 1 /* Difference */;
...
var _Calculator = class {
  static getSet(sets, name) {
  ...
  static calcForce(a, b, distance) {
    const forceDirection = { x: (a.x - b.x) / distance, y: (a.y - b.y) / distance };
    const clampedDistance = Math.max(distance, 0.1);
    const forceMagnitude = _Calculator.attractionStrength / (clampedDistance * clampedDistance);
    return { x: forceDirection.x * forceMagnitude, y: forceDirection.y * forceMagnitude };
  }
  static iterate_positions(sets, relations) {
    for (let i = 0; i < relations.length; i++) {
      const relation = relations[i];
      const a = _Calculator.getSet(sets, relation.a);
      const b = _Calculator.getSet(sets, relation.b);
      let force = { x: 0, y: 0 };
      const distance = _Calculator.calcDistance(a, b);
      if (!relation.shouldIntersect) {
        if (distance < radius * 2.5) {
          force = _Calculator.calcForce(a, b, distance);
          force.x *= -1;
          force.y *= -1;
        }
      } else {
  ...
  static generatePoses(data, w, h) {
    ...
    let iter = 0;
    while (iter < 220) {
      [sets, relations] = _Calculator.iterate_positions(sets, relations);
      iter++;
    }
    let minX = Infinity;
    let minY = Infinity;
...
Calculator.attractionStrength = Math.pow(10, 5);
var InterweavePlugin = class extends import_obsidian.Plugin {
  makeString() {
    let outString = "";
  ...
  async onload() {
    this.registerMarkdownCodeBlockProcessor("interweave", (source, el) => {
      try {
        let diagram = load(source, { json: true });
